...
const PostAlbum = ({ t, theme, post }) => {
  const styling = styles(theme)
  const navigation = useNavigation()

  const colorFrom = path(['image', 'colors', '4'])(post)
  const gradientFrom = colorFrom ? `rgba(${colorFrom.r}, ${colorFrom.g}, ${colorFrom.b}, 0.6)` : '#03a9f4'

  const colorTo = path(['image', 'colors', '0'])(post)
  const gradientTo = colorTo ? `rgba(${colorTo.r}, ${colorTo.g}, ${colorTo.b}, 0.6)` : '#009688'
