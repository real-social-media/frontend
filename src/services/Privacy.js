...
export const postCommentVisibility = (post, user) =>
  post.commentsDisabled === false && !path(['postedBy', 'commentsDisabled'])(post) && !path(['commentsDisabled'])(user)
...
export const postShareVisibility = (post, user) =>
  ...
  (path(['textTaggedUsers'])(post) || []).find((textTag) => textTag.tag === `@${path(['username'])(user)}`)
...
export const postVerificationVisibility = (post) =>
  !postRepostVisiblity(post) && path(['isVerified'])(post) === false && path(['postType'])(post) !== 'TEXT_ONLY'

/**
 *
 */
export const selfPostVerificationVisibility = (post, user) =>
  user.userId === path(['postedBy', 'userId'])(post) && postVerificationVisibility(post)

/**
 *
 */
export const postExpiryVisiblity = (post) =>
  !postRepostVisiblity(post) && !postVerificationVisibility(post) && path(['expiresAt'])(post)
...
export const userFollowingVisibility = (user) =>
  ...
  (path(['followCountsHidden'])(user) !== true || path(['followedStatus'])(user) === 'SELF') &&
  !(path(['followedStatus'])(user) === 'NOT_FOLLOWING' && path(['privacyStatus'])(user) === 'PRIVATE')
